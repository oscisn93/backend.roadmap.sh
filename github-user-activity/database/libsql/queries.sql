...
SELECT * FROM ref_create
    WHERE id = ?
        LIMIT 1;

-- name: GetRefCreates :many
SELECT * FROM ref_create
    ORDER BY pusher_type
        RETURNING *;

-- name: AddRefCreate :one
INSERT INTO ref_create(
    ref,
    ref_type,
    master_branch,
    description,
